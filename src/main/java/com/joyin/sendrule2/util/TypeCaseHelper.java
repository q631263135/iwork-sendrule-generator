...
import com.joyin.platform.common.exception.TypeCastException;
...
public class TypeCaseHelper {
	...
	public static Object convert(Object obj, String type, String format) throws TypeCastException {
		...
		if (obj instanceof BigDecimal) {
			...
			if ("String".equals(type) || String.class.getName().equals(type))
				if(bigD==null){//为了防止金额指标和比例指标数据有误差 所以这里去掉金额格式化  zxj
					return "";
				}else{
			...
			if ("Float".equals(type) || Float.class.getName().equals(type))
				return new Float(bigD.floatValue());
			if ("Long".equals(type) || Long.class.getName().equals(type))
				return new Long(Math.round(bigD.doubleValue()));
			if ("Integer".equals(type) || Integer.class.getName().equals(type))
				...
				try {
					...
					try {
						...
						try {
							...
							e.printStackTrace();
							new TypeCastException("Conversion from " + obj.getClass().getName() + " to " + type + " not currently supported");
						}
					}
	...
	public static BigDecimal convert2Decimal(Object obj) throws TypeCastException {
	...
	public static Date convert2Date(Object obj, String format) throws TypeCastException {
		return (Date) convert(obj, "Date", format);
	}

	public static Date convert2Date(Object obj) throws TypeCastException {
		return (Date) convert(obj, "Date", DateUtil.Format_Date);
	}
